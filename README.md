...
**重要：** 系统会自动将第一个注册的用户设置为超级管理员 (`super_admin`)。这意味着：

- 第一个通过任何方式（邮箱注册、OAuth 登录等）注册的用户将自动获得超级管理员权限
- 超级管理员可以访问 `/dashboard/admin` 路径下的所有管理功能
- 超级管理员可以管理其他用户、查看系统统计信息、处理订阅等

**建议的设置流程：**

1. 部署应用后，立即使用您的主要邮箱地址注册第一个账户
2. 确认该账户已获得超级管理员权限（可在用户资料或管理面板中查看）
3. 后续的用户注册将默认获得普通用户权限

**安全提示：**

- 请确保第一个注册的账户使用安全的邮箱地址
- 建议在生产环境部署后立即注册管理员账户，避免被他人抢先注册
