    ...
    <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle>Current Plan: {subscription.tierId.charAt(0).toUpperCase() + subscription.tierId.slice(1)}</CardTitle>
                <CardDescription>Your active plan details.</CardDescription>
              </div>
              <Badge
                variant={['active', 'trialing'].includes(subscription.status) ? 'default' : 'destructive'}
          ...
          <CardContent>
            ...
            <Button className="mt-4" onClick={handleManageSubscription} disabled={isPortalLoading}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>View your recent purchases and subscription payments</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {payments.map((payment) => (
                  <TableRow key={payment.id}>
                    ...
                    <TableCell>
                      <Badge variant={payment.paymentType === 'one_time' ? 'secondary' : 'default'}>
                        {payment.paymentType === 'one_time' ? 'One Time Purchase' : 'Subscription'}
                      </Badge>
                    </TableCell>
                    <TableCell>
