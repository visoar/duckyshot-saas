...
jest.mock("./dashboard-page-header", () => ({
  DashboardPageHeader: ({ 
    ...
    <header data-testid="dashboard-page-header">
      ...
      {parentTitle && <span data-testid="header-parent-title">{parentTitle}</span>}
      {parentUrl && <span data-testid="header-parent-url">{parentUrl}</span>}
      {description && <span data-testid="header-description">{description}</span>}
      {actions && <div data-testid="header-actions">{actions}</div>}
      <span data-testid="header-sidebar-trigger">{showSidebarTrigger ? "true" : "false"}</span>
    </header>
  ),
}));
...
describe("DashboardPageWrapper", () => {
  ...
  it("should pass all props to DashboardPageHeader", () => {
    ...
    expect(screen.getByTestId("header-parent-title")).toHaveTextContent("Parent Page");
    expect(screen.getByTestId("header-parent-url")).toHaveTextContent("/parent");
    expect(screen.getByTestId("header-description")).toHaveTextContent("Test description");
    expect(screen.getByTestId("header-actions")).toBeInTheDocument();
    expect(screen.getByTestId("custom-action")).toBeInTheDocument();
    expect(screen.getByTestId("header-sidebar-trigger")).toHaveTextContent("false");
  });
  ...
  it("should render children in main element", () => {
    ...
    expect(screen.getByTestId("main-heading")).toHaveTextContent("Main Content");
    expect(screen.getByTestId("main-paragraph")).toHaveTextContent("This is the main content area.");
  });
  ...
  it("should default showSidebarTrigger to true", () => {
    ...
    expect(screen.getByTestId("header-sidebar-trigger")).toHaveTextContent("true");
  });
  ...
  it("should render with only title and children", () => {
    ...
    expect(screen.getByTestId("header-title")).toHaveTextContent("Minimal Title");
    expect(screen.getByTestId("minimal-content")).toHaveTextContent("Minimal Content");
