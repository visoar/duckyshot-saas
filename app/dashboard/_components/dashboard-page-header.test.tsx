...
jest.mock("@/components/ui/breadcrumb", () => ({
  ...
  BreadcrumbPage: ({ children, className }: { children: React.ReactNode; className?: string }) => (
    <span data-testid="breadcrumb-page" className={className}>{children}</span>
  ),
  BreadcrumbSeparator: ({ className }: { className?: string }) => (
    <span data-testid="breadcrumb-separator" className={className}>/</span>
  ),
}));
...
jest.mock("@/components/mode-toggle", () => ({
  ModeToggle: ({ variant, size }: { variant?: string; size?: string }) => (
    <button data-testid="mode-toggle" data-variant={variant} data-size={size}>Toggle</button>
  ),
}));
...
describe("DashboardPageHeader", () => {
  it("should render with required props", () => {
    ...
    expect(screen.getByTestId("breadcrumb-page")).toHaveTextContent("Test Title");
  ...
  it("should render with parent title and URL", () => {
    ...
    expect(screen.getByTestId("breadcrumb-link")).toHaveTextContent("Parent Page");
    expect(screen.getByTestId("breadcrumb-link")).toHaveAttribute("href", "/parent");
    expect(screen.getByTestId("breadcrumb-separator")).toBeInTheDocument();
    expect(screen.getByTestId("breadcrumb-page")).toHaveTextContent("Child Page");
  });
