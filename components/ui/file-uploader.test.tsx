...
global.XMLHttpRequest = jest.fn(() => mockXHR) as jest.MockedClass<typeof XMLHttpRequest>;
...
describe('FileUploader Performance', () => {
  ...
  it('should efficiently update progress without re-rendering entire list', async () => {
    ...
    mockXHR.upload.addEventListener.mockImplementation((event: string, callback: (event: ProgressEvent) => void) => {
      if (event === 'progress') {
  ...
  it('should handle multiple file uploads efficiently', async () => {
    ...
    render(<FileUploader maxFiles={3} onUploadComplete={mockOnUploadComplete} />);
