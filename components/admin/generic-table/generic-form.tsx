...
import { useForm, type FieldValues, ControllerRenderProps } from "react-hook-form";
import { z } from "zod";
...
export function GenericForm({
    ...
    const validationSchema = useMemo(() => generateZodSchema(schemaInfo, readOnlyColumns), [schemaInfo, readOnlyColumns]);
    ...
    const renderField = (field: ControllerRenderProps<FieldValues, string>, col: ColumnInfo) => {
        ...
        if (readOnlyColumns.includes(col.name)) {
            return <Input {...field} value={String(value)} readOnly disabled className="bg-muted" />;
        }
        ...
        switch (col.type) {
            ...
            case 'date':
                return <Input type="datetime-local" {...field} value={value ? new Date(value).toISOString().slice(0, 16) : ''} />;
            case 'json':
                return <Textarea {...field} value={typeof value === 'object' && value !== null ? JSON.stringify(value, null, 2) : String(value)} />;
            default:
