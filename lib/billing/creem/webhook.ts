...
import { findUserByCustomerId, upsertSubscription, upsertPayment, isWebhookEventProcessed, recordWebhookEvent } from "@/lib/database/subscription";
...
function getCustomerId(customerField: string | { id: string } | undefined): string {
    if (!customerField) {
...
function isSubscriptionObject(obj: unknown): obj is CreemSubscriptionObject {
    return typeof obj === 'object' && obj !== null && 'current_period_end_date' in obj;
}
function isPaymentObject(obj: unknown): obj is CreemPaymentObject {
    return typeof obj === 'object' && obj !== null && ('amount' in obj || 'amount_paid' in obj);
}
...
function verifyCreemSignature(payload: string, signature: string, secret: string): boolean {
    ...
    try {
        ...
        console.warn("Error during signature comparison (likely non-matching lengths):", error);
        return false;
    }
}
    ...
    await db.transaction(async (tx) => {
        ...
        switch (event.eventType) {
            case "checkout.completed":
                if (isCheckoutObject(eventObject)) await processCheckoutCompletedEvent(eventObject, tx);
                break;
            case "payment.succeeded":
                if (isPaymentObject(eventObject) && eventObject.billing_reason === 'subscription_cycle') {
            ...
            case "subscription.canceled":
                if (isSubscriptionObject(eventObject)) await processSubscriptionEvent(eventObject, tx);
                break;
            ...
            case "subscription.paid":
                ...
                break;
            default:
                console.log(`Ignoring unhandled Creem webhook event type: ${event.eventType}`);
        }
    });
...
async function processCheckoutCompletedEvent(checkoutData: CreemCheckoutObject, tx: Tx) {
    const { subscription, customer: customerField, metadata, order } = checkoutData;
    if (!customerField || !order) {
        throw new Error("checkout.completed event is missing required data objects (customer or order).");
    }
    const userId = metadata?.userId as string | undefined;
    if (!userId) {
        throw new Error(`userId not found in metadata for checkout ${checkoutData.id}`);
    }

    const customerId = getCustomerId(customerField);
    await tx.update(users).set({ paymentProviderCustomerId: customerId }).where(eq(users.id, userId));
    ...
    if (subscription && paymentMode === 'subscription') {
        const productId = typeof subscription.product === 'string' ? subscription.product : subscription.product.id;
        ...
        await upsertPayment({
            ...
            amount: order.amount_due, currency: order.currency,
            status: "succeeded",
    ...
    // Handle one_time purchases
        ...
        throw new Error(`Unsupported payment mode: ${paymentMode} or missing subscription data for subscription mode`);
    }
}

async function processSubscriptionEvent(subscriptionData: CreemSubscriptionObject, tx: Tx) {
    const customerId = getCustomerId(subscriptionData.customer);
    const user = await findUserByCustomerId(customerId, tx);
    if (!user) throw new Error(`User not found for customerId ${customerId} on subscription event.`);

    const productId = typeof subscriptionData.product === 'string' ? subscriptionData.product : subscriptionData.product.id;
    const tier = getProductTierByProductId(productId);

    await upsertSubscription({
        userId: user.id, customerId, subscriptionId: subscriptionData.id,
        productId: tier?.id || productId,
...
async function processSubscriptionRenewal(renewalData: CreemPaymentObject | CreemSubscriptionObject, tx: Tx) {
    const customerId = getCustomerId(renewalData.customer);
    const subscriptionId = 'subscription_id' in renewalData && renewalData.subscription_id ? renewalData.subscription_id : renewalData.id;

    if (!subscriptionId) throw new Error("Subscription ID missing in renewal event");

    const user = await findUserByCustomerId(customerId, tx);
    if (!user) throw new Error(`User not found for customerId ${customerId} during subscription renewal.`);

    let periodStartDateStr: string, periodEndDateStr: string;

    if (isPaymentObject(renewalData) && renewalData.lines?.data?.[0]?.period) {
        periodStartDateStr = new Date(renewalData.lines.data[0].period.start * 1000).toISOString();
        periodEndDateStr = new Date(renewalData.lines.data[0].period.end * 1000).toISOString();
        ...
        throw new Error("Could not determine new period for subscription renewal from event object.");
    }

    let productId: string;
    if (isPaymentObject(renewalData)) {
        productId = renewalData.product_id || (renewalData.lines?.data?.[0]?.price?.product ?? '');
    } else { // isSubscriptionObject
        productId = typeof renewalData.product === 'string' ? renewalData.product : renewalData.product.id;
    }
    ...
    await upsertSubscription({
        userId: user.id, customerId, subscriptionId,
        productId: tier?.id || productId,
        status: 'active',
...
async function processPaymentSucceededEvent(paymentData: CreemPaymentObject, tx: Tx) {
    const customerId = getCustomerId(paymentData.customer);
    const user = await findUserByCustomerId(customerId, tx);
    if (!user) throw new Error(`User not found for customerId ${customerId} during payment processing.`);

    const productId = paymentData.product_id || paymentData.lines?.data?.[0]?.price?.product;
    ...
    await upsertPayment({
        ...
        status: "succeeded",
        paymentType: (paymentData.metadata?.paymentMode as string) || 'subscription',
    }, tx);
}