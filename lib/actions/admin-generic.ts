...
export async function getGenericTableData(
...
): Promise<{ data: RecordWithId[]; pagination: { page: number; limit: number; total: number; totalPages: number; } }> {
    ...
    const dataQuery = db.select().from(table).where(finalWhere).limit(limit).offset(offset);
    const totalQuery = db.select({ total: count() }).from(table).where(finalWhere);
...
const tableNameSchema = z.custom<EnabledTableKeys>(
    (val) => Object.keys(enabledTablesMap).includes(val as string),
);
...
export const createRecord = adminAction
    ...
    .action(async ({ parsedInput: { tableName, data } }) => {
        ...
        const [newRecord] = await db.insert(table).values(cleanData as never).returning();
