...
function createMockFile(content: Uint8Array | string, mimeType: string, filename: string): File {
...
describe('File Security Validation', () => {
  describe('validateFileSignature', () => {
    ...
    it('should reject text files with null bytes', async () => {
      const binaryData = new Uint8Array([0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x00, 0x57, 0x6F]);
  ...
  describe('scanForMaliciousContent', () => {
    it('should detect script injection in content', async () => {
      ...
      const file = createMockFile(maliciousContent, 'text/html', 'malicious.html');
      
      const result = await scanForMaliciousContent(file);
      
      expect(result.isSafe).toBe(false);
      expect(result.threats.some(threat => threat.includes('script'))).toBe(true);
    });
    ...
    it('should detect javascript: URLs', async () => {
      ...
      expect(result.isSafe).toBe(false);
      expect(result.threats.some(threat => threat.includes('javascript:'))).toBe(true);
    });
    ...
    it('should detect vbscript: URLs', async () => {
      ...
      expect(result.isSafe).toBe(false);
      expect(result.threats.some(threat => threat.includes('vbscript:'))).toBe(true);
    });
    ...
    it('should detect executable file headers', async () => {
      ...
      const file = createMockFile(executableContent, 'application/octet-stream', 'malware.exe');
      
      const result = await scanForMaliciousContent(file);
      
      expect(result.isSafe).toBe(false);
      expect(result.threats.some(threat => threat.includes('Executable file content'))).toBe(true);
    });
  ...
  describe('edge cases and error handling', () => {
    ...
    it('should handle malicious content scanning errors gracefully', async () => {
      ...
      expect(result.isSafe).toBe(false);
      expect(result.threats.some(threat => threat.includes('Error scanning file content'))).toBe(true);
    });
  });
});